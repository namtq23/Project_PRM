...
class _EditTourScreenState extends ConsumerState<EditTourScreen> {
  ...
  Future<void> _submit() async {
    ...
    final success = await ref.read(toursViewModelProvider.notifier).updateTour(
          id: widget.tour.id!,
          title: title,
          description: desc,
          price: price,
  ...
  Widget _buildGlassCard({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: ToursTheme.outlineVariant.withOpacity(0.4),
          width: 1,
        ),
      ),
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            ...
            decoration: const InputDecoration(
              ...
              labelStyle: TextStyle(color: ToursTheme.onSurfaceVariant),
              border: OutlineInputBorder(),
              prefixIcon: Icon(Icons.speed, size: 20, color: ToursTheme.onSurfaceVariant),
            ),
            dropdownColor: ToursTheme.surfaceContainerHigh,
            style: const TextStyle(color: Colors.white),
            items: const [
              DropdownMenuItem(value: 'Dễ', child: Text('Dễ (Easy)')),
              DropdownMenuItem(value: 'Trung bình', child: Text('Trung bình (Moderate)')),
          ...
          TextFormField(
            ...
            style: const TextStyle(color: Colors.white),
            maxLines: 5,
            validator: (val) => val == null || val.isEmpty ? 'Vui lòng nhập mô tả chi tiết' : null,
          ),
        ],
      ),
    );
  ...
  Widget _buildPerformanceCard() {
    return _buildGlassCard(
      ...
      child: Column(
        children: [
          _buildPerformanceRow('Tỷ lệ đặt chỗ (Booking Rate)', '94%', Icons.check_circle_outline, Colors.green),
          const Divider(color: ToursTheme.outlineVariant, height: 20),
          _buildPerformanceRow('Điểm đánh giá trung bình', '4.9 ★', Icons.star_border, Colors.amber),
          const Divider(color: ToursTheme.outlineVariant, height: 20),
          _buildPerformanceRow('Lượt xem tháng này', '1,240 lượt', Icons.visibility_outlined, ToursTheme.primary),
        ],
      ),
    );
  }
    ...
    return Row(
      children: [
        ...
        Expanded(
          child: Text(
            label,
            style: const TextStyle(color: ToursTheme.onSurfaceVariant, fontSize: 13),
          ),
        ),
        Text(
          value,
  ...
  Widget _buildItineraryCard() {
    return _buildGlassCard(
      ...
      child: Column(
        children: [
          _buildItineraryStep('Ngày 1', 'Khởi hành & Check-in', 'Di chuyển đến khu nghỉ dưỡng, đón đoàn và tổ chức tiệc chào mừng.', true),
          _buildItineraryStep('Ngày 2', 'Khám Phá Điểm Đến', 'Tham quan các kỳ quan chính, trải nghiệm các trò chơi mạo hiểm dã ngoại.', true),
          _buildItineraryStep('Ngày 3', 'Mua Sắm & Trở Về', 'Thời gian hoạt động tự do mua sắm quà lưu niệm và xe đưa tiễn ra sân bay.', false),
        ],
      ),
    );
  }

  Widget _buildItineraryStep(String day, String title, String description, bool hasNext) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Column(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: ToursTheme.primary.withOpacity(0.5)),
              ),
              child: Text(
                day,
                style: const TextStyle(color: ToursTheme.primary, fontWeight: FontWeight.bold, fontSize: 11),
              ),
            ),
            if (hasNext)
              Container(
